...
**Last updated:** 2026-08-11 — **🎉 STOCKLY IS LIVE ON PIRO. Plus: post-qualification minimums built (not deployed), and two long-held beliefs corrected.** Full journal: `progress/2026-08-11-piro-live-and-post-qualification-minimums.md`.
  ...
  - **⏳ Next:** deploy needs BOTH `railway up` and `shopify app deploy --config=piro`, each through `deployment-guardian`. Then the real B2B checkout test on Piro **before** telling Ana the minimum is live. Currency fixed only on 2 screens (~50 hardcoded `€` sites remain elsewhere).
