...
**Last updated:** 2026-06-01 — RF editor **Live preview now renders inside the App Bridge max modal** (was blank): `chrome="modal"` swaps Polaris `<Layout>` for an explicit 2-col CSS grid; `chrome="page"` unchanged. LIVE Fly v66 (admin-UI only — no `shopify app deploy`). ⏳ Pending Jonatan's visual confirmation in the dev store. Read-only RESCUE AUDIT still open (see `tasks/current.md` + `progress/2026-05-29-rescue-deep-read-and-interview.md`)
