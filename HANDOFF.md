...
**Last updated:** 2026-05-30 — RF editor in a full-canvas App Bridge **max modal**, with edit/delete/add now working via INLINE panels (LIVE, Fly v64). Read-only RESCUE AUDIT still open (see `tasks/current.md` + `progress/2026-05-29-rescue-deep-read-and-interview.md`)
**Last commit:** `62d9b27` Revert cli bump (see ⚠️ below); the production feature commit is `233d6df` fix(registration-form): inline field panels.
**GitHub:** https://github.com/creativedesignseo/stockly
**Production URL:** https://stockly-lustrous-forest-4364.fly.dev
**Fly version:** `v64` (manual `fly deploy` 2026-05-30 — field edit/delete/add inside the max modal converted from floating Polaris modals to INLINE middle-pane panels; the modals portaled behind the max-modal overlay and looked dead). `v63` shipped the max modal itself. Both pure admin-UI, `fly deploy` only.
