...
**Shopify app version:** `stockly-27` (storefront `registration-form.js` now POSTs `__shortcode` on submit so the server validates against the exact form shown. `stockly-26` added the `form_shortcode` block setting; dual-serve back-compat. ⚠️ `registration-form.js` still >10 KB app-block threshold — non-blocking, trim later)
