...
Split out of `tasks/current.md` on 2026-08-14, which had grown to 893 lines
despite its own header asking for "a single page of what's being worked on
right now".

Everything below is **done, superseded, or stale backlog** going back to
2026-05. Kept for the reasoning, not as a to-do list. Do not work from this
file — `tasks/current.md` is the queue.

---

## Historical — the submission checklist (all done)

### Listing tasks, completed
...
- [x] **Run `shopify app deploy` — DONE 2026-07-07.** `stockly-43` released
  ...
  Fly→Railway URLs + the Discount Function `shop.localTime` clock-bug fix.
  Gated through `deployment-guardian` (GO verdict) before running.
