# HANDOFF archive — entries closed before 2026-08-12

Split out of `HANDOFF.md` on 2026-08-14. Everything here is **finished
history**, kept because the reasoning is often the only record of why
something is the way it is. The live operational state is in `HANDOFF.md`;
per-session journals are in `progress/`.

Read this only when you need the backstory of a decision. It is deliberately
not loaded at session start.

---

Prior 2026-08-12 — **✅ ALL FOUR Shopify Level 2 protected-customer-data requirements are MET and ACCEPTED (16/16, no rejection). App Store registration paid. Listing category, tags and language saved. The screencast, screenshots, demo store/reviewer credentials, legal review and manual pasting of listing text remain pending.** Read `docs/decisions/ADR-018-*` for why the public app was necessary, `docs/app-store-listing-content.md` for the ready-to-paste listing copy, and `docs/app-store-submission-guide.md` for the full task list.

  - **Verified 2026-08-12, without deploying:** `npx tsc --noEmit` passes on the current worktree. The Railway production app at `https://stockly-production-5ccf.up.railway.app` returns HTTP 200 for `/healthz`, `/`, `/legal/privacy` and `/legal/terms`. This verifies reachability and those routes only; it does not verify an authenticated Shopify merchant flow or App Store approval.
  - **Local-only media work is not production state:** the Remotion promo project and its 4K render live under `Promo/Video/` in the worktree. They have not been deployed to Railway or Shopify. The 45-second promo is not the required Shopify screencast, which must demonstrate the app workflow and remains pending.
...
Also 2026-08-12 — **🌐 The marketing site is LIVE at https://stocklygo.netlify.app** (commit `3313ddf`, source in `site/`). Second thing this project now runs in production, independent of the Railway app.
  ...
  - **✅ `https://stocklygo.site` is LIVE** (done 2026-08-12, same session). DNS is Cloudflare, zone `454d8129b42920d670ac21d47ee7b895` under `Ruth.tovar4@gmail.com's Account` (id `2db1294827f25d7c3fb9e169fc2c4a43`) — **the same account that holds `lowsplit.com`**. The reason the existing `lowsplit` token could not touch it is that the token is scoped to a single *zone*, not that the zones live in different accounts. Cloudflare tokens are per-zone unless explicitly created for "All zones from an account". Records created: `A @ → 75.2.60.5` and `CNAME www → stocklygo.netlify.app`, both **DNS only (grey cloud)** — Cloudflare's orange proxy would block Netlify's Let's Encrypt validation. Verified live: apex 200 serving the real page, `www` 301 → apex, certificate `CN=stocklygo.site` valid to 2026-11-10.
